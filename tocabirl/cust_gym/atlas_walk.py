...
class AtlasEnv(atlas_walk_env.AtlasEnv):
    ...
    def _get_obs(self):
        ...
        return (cur_obs - self.obs_mean) / np.sqrt(self.obs_var + 1e-8)
    ...
    def step(self, a):
        ...
        qpos_ordered = np.concatenate([qpos[23:], qpos[7:23]])
        target_qpos_ordered = np.concatenate([target_data_qpos[0:12], self.init_q_desired[7:23]])

        mimic_body_orientation_reward =  0.3 * exp(-13.2*abs(baseQuatError)) 
        qpos_regulation = 0.35*exp(-4.0*(np.linalg.norm(target_qpos_ordered - qpos_ordered)**2))
        ...
        if ((self.mocap_data_idx < 300) or \
            ...
            double_support_force_diff_regulation = 0.1*exp(-0.005*(np.linalg.norm(self.ft_right_foot - self.ft_left_foot)))
        ...
        reward = mimic_body_orientation_reward + qpos_regulation + qvel_regulation + contact_force_penalty + torque_regulation + torque_diff_regulation + qacc_regulation + body_vel_reward + foot_contact_reward + contact_force_diff_regulation + double_support_force_diff_regulation
    ...
    def reset_model(self):
        ...
        self.target_vel = np.array([0.2, 0.0])
