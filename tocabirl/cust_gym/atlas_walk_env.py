...
class AtlasEnv(gym.envs.mujoco.MujocoEnv):
    ...
    def __init__(self, model_path, frame_skip):
        ...
        self.nominal_body_pos = np.copy(self.model.body_pos)
        self.nominal_dof_damping = np.copy(self.model.dof_damping)
        self.nominal_dof_frictionloss = np.copy(self.model.dof_frictionloss)

        # motor_constant_scale = np.ones_like(np.random.uniform(0.95, 1.05, 6))
        # self.motor_constant_scale = np.tile(motor_constant_scale, 2)

        self.body_mass_scale = 1.0
        self.body_inertia_scale = 1.0
        self.body_ipos_scale = 1.0
        self.dof_frictionloss_scale = 1.0
        self.dof_damping_scale = 1.0
        self.motor_constant_scale = 1.0
        self.leg_length_scale = 1.0
