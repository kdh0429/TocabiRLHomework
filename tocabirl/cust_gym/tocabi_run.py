...
class DYROSTocabiEnv(tocabi_run_env.TocabiEnv):
    ...
    def step(self, a):
        ...
        target_data_qpos = cubic(local_time_plus_init, self.mocap_data[self.mocap_data_idx,0], self.mocap_data[next_idx,0], self.mocap_data[self.mocap_data_idx,8:41], self.mocap_data[next_idx,8:41], 0.0, 0.0)
        ...
        if ((abs(self.ft_left_foot[2]) > 1.6 * 9.81 * sum(self.model.body_mass)) or (abs(self.ft_right_foot[2]) > 1.6 * 9.81 * sum(self.model.body_mass))):
        ...
        reward = mimic_body_orientation_reward + qpos_regulation + qvel_regulation + contact_force_penalty + torque_regulation + torque_diff_regulation + qacc_regulation + body_vel_reward + foot_contact_reward + contact_force_diff_regulation + force_thres_penalty + force_diff_thres_penalty
    ...
    def reset_model(self):
        ...
        self.target_vel = np.array([np.random.uniform(0.0, 1.0), np.random.uniform(-0.2, 0.2)])
